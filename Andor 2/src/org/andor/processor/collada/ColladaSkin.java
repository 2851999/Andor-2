...
public class ColladaSkin {
	...
	public ColladaBindShapeMatrix bindShapeMatrix;
	...
	public void parse(Node parent) {
		...
		for (int a = 0; a < nodes.getLength(); a++) {
			...
			if (node.getNodeName().equals("source")) {
				...
				this.joints = new ColladaJoints();
				this.joints.parse(node);
			} else if (node.getNodeName().equals("vertex_weights")) {
				this.vertexWeights = new ColladaVertexWeights();
				this.vertexWeights.parse(node);
			}
		}
	}
