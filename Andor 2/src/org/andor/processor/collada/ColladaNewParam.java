...
public class ColladaNewParam {
	...
	public void parse(Node parent) {
		...
		for (int a = 0; a < nodes.getLength(); a++) {
			...
			if (node.getNodeName().equals("sampler2D")) {
				this.sampler2D = new ColladaSampler2D();
				this.sampler2D.parse(node);
			} else if (node.getNodeName().equals("surface")) {
				this.surface = new ColladaSurface();
				this.surface.parse(node);
			}
		}
	}
